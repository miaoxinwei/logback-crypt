...
import com.lico.logback.crypt.utils.StreamUtil;
...
public class RSAEncryptor extends BaseAsymmetricEncryptor {
    ...
    protected String getSignAlgorithm() {
        return Constants.SIGN_ALGORITHMS;
    }

    protected String getAsymmetricType() {
        return Constants.ENCRYPT_TYPE_RSA;
    }
    ...
    protected String doDecrypt(String cipherTextBase64, String charset, String privateKey) throws Exception {
        ...
        PrivateKey priKey = getPrivateKeyFromPKCS8(Constants.ENCRYPT_TYPE_RSA,
                new ByteArrayInputStream(privateKey.getBytes()));
        Cipher cipher = Cipher.getInstance(Constants.ENCRYPT_TYPE_RSA);
        cipher.init(Cipher.DECRYPT_MODE, priKey);
        byte[] encryptedData = StringUtils.isEmpty(charset)
                ? Base64.getDecoder().decode(cipherTextBase64.getBytes())
                : Base64.getDecoder().decode(cipherTextBase64.getBytes(charset));
    ...
    protected String doEncrypt(String plainText, String charset, String publicKey) throws Exception {
        ...
        PublicKey pubKey = getPublicKeyFromX509(Constants.ENCRYPT_TYPE_RSA,
